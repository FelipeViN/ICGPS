...
@Component({
  selector: 'app-grupos',
  standalone: true,
  imports: [ReactiveFormsModule, CommonModule, FormsModule, ToolbarComponent],
...
export class GruposComponent {
  ...
  agregarGrupo(): void {
    if (this.grupoForm.valid) {
      ...
      const nuevoGrupo: Grupos = {
        id: 1, // Asignar un valor nulo o manejarlo según tu lógica
        ...
        creationAt: new Date().toISOString(), // Asigna la fecha actual o maneja según tu lógica
    ...
    if (this.grupoForm.valid) {
      ...
      if (id) {
        const grupoActualizado: Grupos = {
          ...
          semestre: Number(this.grupoForm.get('semestre')?.value) || 1,
          cicloEscolar: this.grupoForm.get('cicloEscolar')?.value || '',
          creationAt: new Date().toISOString(), // Mantener la fecha actual o manejarla según tu lógica
  ...
  get cicloEscolarError(): string { 
    ...
    return control?.hasError('required') ? 'El ciclo escolar es obligatorio' : ''; 
  }
